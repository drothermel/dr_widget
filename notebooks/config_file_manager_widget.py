...
@app.cell(hide_code=True)
def _():
    he_switch = mo.ui.switch(label="harvest_window", value=False)
    mo.vstack(
        [
            mo.md(
...
def _(he_switch, widget):
    ...
    _field = "harvest_window"
    current_value = (_curr_cfg or {}).get(_field, None)

    if current_value is None:
        mo.output.append(
            mo.md(
                "_Load a config with `selections` data before using the highlight toggle._"
            )
        )
    else:
